...
class IPublicationProvider(Interface):
    ...
    """
    publication_items = schema.List(
        title=u'Publication Items',
        required=True,
        readonly=True
    )
...
class IBasicPublicationSupport(Interface):
    ...
    """
    support_enabled = schema.Bool(
        title=u'Publication Support Enabled?',
        required=True,
        readonly=True
    )
...
class IAdditionalPublicationInfo(Interface):
    """ Marker for adapter that provides additional info in the publication
    view.
    """
